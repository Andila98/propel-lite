    ...
    try {
        ...
        const priorityPromises = requestsToPrioritize.map(async (request) => {
            try {
                ...
                console.error(`[API_MAINTENANCE_AI_ERROR] AI prioritization failed for request ${request.id}:`, aiError);
        ...
        console.error('[API_MAINTENANCE_ERROR] Failed to get maintenance requests:', error);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        console.error('[API_MAINTENANCE_ERROR] Failed to create maintenance request:', error);
