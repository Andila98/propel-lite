...
// Safe formatters
const safeToFixed = (value: any, decimals = 1) => {
...
const safeCurrency = (value: any, currency = 'USD') => {
  if (value === null || value === undefined || isNaN(value)) {
...
const safePercentage = (value: any) => {
  ...
  // API might return percentage as 0-100, not 0-1
...
function AiInsightsCard({ summary, anomalies }: { summary: string | undefined, anomalies: any[] | undefined }) {
  return (
    <Card className="lg:col-span-2 relative overflow-hidden bg-gradient-to-br from-purple-500/10 via-pink-500/5 to-orange-500/10 border-purple-500/20 group hover:shadow-2xl transition-all duration-500">
      ...
      <CardContent className="relative z-10">
        <p className="text-sm text-foreground/80 mb-4 leading-relaxed">{summary || "Generating insights..."}</p>
          ...
          <div className="space-y-2">
            ...
            {anomalies.slice(0, 2).map((alert, index) => (
              <div key={index} className="flex items-center space-x-3 p-3 rounded-lg bg-card/50 border border-border/50 backdrop-blur-sm">
                <div className={`h-2 w-2 rounded-full ${alert.type === 'vacancy-rate' ? 'bg-orange-500' : 'bg-red-500'} animate-pulse`} />
...
function PropertyShowcase({ properties }: { properties: any[] | undefined }) {
            ...
            <div key={property.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors duration-200 group">
              <div className="flex items-center space-x-3">
                <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-semibold">
                  {property.name ? property.name.charAt(0) : 'P'}
                </div>
                <div>
...
export default function DashboardPage() {
  ...
  const renderContent = () => {
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-64 text-center p-4 rounded-lg bg-destructive/10 border border-destructive/20">
                <WifiOff className="h-12 w-12 mb-4 text-destructive" />
    ...
    if (!data) {
        return <div className="text-center py-10 text-muted-foreground">No dashboard data available.</div>
    }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <ChartCard title="Late Payment Trends">
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.latePaymentData}>
                  <defs>
