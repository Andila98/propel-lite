...
export async function GET(req: NextRequest) {
    ...
    // Use getLandlordAndActor to handle session verification and get user info
    ...
    try {
        ...
        const properties: Property[] = propertiesSnapshot.docs.map((doc) => {
            ...
            if (propertyData.units.length > 0) {
                ...
                // Safely parse bedrooms, default to 0 if not a number
                const bedrooms = parseInt(firstUnit.size, 10);
                propertyData.bedrooms = isNaN(bedrooms) ? 0 : bedrooms;
